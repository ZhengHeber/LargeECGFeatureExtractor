...
def load_and_process_data(idx, df, data_path):
    ...
    return normalized_data, torch.tensor(label, dtype=torch.float32)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_dataset)):
    ...
    model = TransformerClassifier(num_classes=num_classes).to(device)  # Ensure model is on the correct device
