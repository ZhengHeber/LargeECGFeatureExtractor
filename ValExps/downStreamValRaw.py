...
class SimpleCNN(nn.Module):
    ...
    def _get_conv_output_dim(self):
        dummy_input = torch.zeros(1, 12, 5000).to(device)  # Ensure dummy input is on the correct device
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_dataset)):
    ...
    model = SimpleCNN(num_classes=num_classes).to(device)  # Ensure model is on the correct device
