...
class TransformerClassifier(nn.Module):
    def __init__(self, num_classes=4, d_model=720, nhead=4, num_encoder_layers=4, dim_feedforward=512, dropout=0.1):
    ...
    def _get_trans_output_dim(self):
        dummy_input = torch.zeros(1, 5000, 36)
        ...
        x = rearrange(x, 'b t (p1 f) -> b (t p1) f', t=250, p1=20, f=36)
...
class MambaClass(nn.Module):
    def __init__(self, d_model=720, d_state=64, d_conv=4, num_classes=4, dropout_rate=0.1):
    ...
    def _get_mamba2_output_dim(self):
        dummy_input = torch.zeros(1, 5000, 36).to(self.device)
...
tokenizer_checkpoint = torch.load('/home/zhenghb/HOME/ECGLM/Parameters/best_CrossTokenizer_model_split_100.pth', map_location=device)

cross_tokenizer_state_dict = load_state_dict_without_module_prefix(tokenizer_checkpoint['tokenizer_state_dict'])
...
def load_and_process_data(idx, df, data_path, normecg):
    ...
    normecg = normecg.to(device)

    variance = (data - normecg) ** 2
    ...
    tokenized_data = cross_tokenizer.tokenize(normalized_data.unsqueeze(0)).squeeze(0)

    combined_data = torch.cat([normalized_data, normalized_variance, tokenized_data], dim=1)

    return combined_data, torch.tensor(label, dtype=torch.float32).to(device)

class ECGDataset(Dataset):
    def __init__(self, df, data_path, normecg):
        self.df = df
    ...
    def __getitem__(self, idx):
        if self.data_cache[idx] is None:
            self.data_cache[idx] = load_and_process_data(idx, self.df, self.data_path, self.normecg)
...
dataset = ECGDataset(df, data_path, average_norm_ecg_signal_10)
