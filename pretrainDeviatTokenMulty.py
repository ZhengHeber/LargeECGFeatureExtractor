...
from utils import load_hdf5_file, calculate_model_size, normalize
from mamba_ssm import Mamba2
from tokenizer import DeviatTokenizer  # 修改为DeviatTokenizer
from trainPrepare import train_one_epoch_DEVIAT, test_one_epoch_DEVIAT
...
def main():
    ...
    codebook = np.load(normecg_file_path)
    codebook = torch.tensor(codebook, dtype=torch.float32).cuda()

    # 初始化DeviatTokenizer
    tokenizer = DeviatTokenizer(d_model=256, d_state=64, d_conv=4, dropout_rate=0.1).to(device)
        ...
        for split_idx, split_files in enumerate(split_file_lists):
            ...
            for epoch in range(num_epochs):
                ...
                with tqdm(total=len(train_data_loader), desc=f"Epoch {epoch+1}/{num_epochs}, Split {split_idx+1} Training", disable=(dist.get_rank() != 0)) as pbar_train:
                    train_epoch_loss = train_one_epoch_DEVIAT(model, tokenizer, train_data_loader, loss_function, optimizer, device, pbar=pbar_train, codebook=codebook)
                ...
                with tqdm(total=len(test_data_loader), desc=f"Epoch {epoch+1}/{num_epochs}, Split {split_idx+1} Testing", disable=(dist.get_rank() != 0)) as pbar_test:
                    test_epoch_loss = test_one_epoch_DEVIAT(model, tokenizer, test_data_loader, loss_function, device, pbar=pbar_test, codebook=codebook)
                ...
                if dist.get_rank() == 0 and avg_test_loss < best_test_loss:
                    ...
                    best_model_path = os.path.join(paramDir, f'best_DeviatTokenizer_model_split_{split_idx+1}.pth')
            ...
            if dist.get_rank() == 0:
                final_model_path = os.path.join(paramDir, f'final_DeviatTokenizer_model_split_{split_idx+1}.pth')
